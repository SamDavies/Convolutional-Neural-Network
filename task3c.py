...
fig2, axis_array2 = plt.subplots(2, 5, figsize=(15, 10))
for i in xrange(2):
    for j in xrange(5):
        digit = i * 5 + j
        softmax_weights = model.layers[1].W
        inputs_of_top_unit = numpy.array([input_unit[digit] for input_unit in softmax_weights])
        hinton(inputs_of_top_unit.reshape(10, 10), ax=axis_array2[i, j])
plt.show()